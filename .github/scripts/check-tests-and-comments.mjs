...
const WATCHED_GLOBS = [/^backend\/src\/.+\.ts$/, /^frontend\/src\/.+\.tsx?$/, /^shared\/src\/.+\.ts$/];
...
for (const { status, file } of changedFiles) {
  ...
  const hasTest = candidateTestFiles(file).some((t) => fileExistsAtHead(t) || changedPaths.has(t));
  const hasCommentChange = diffTouchesComment(file);

  if (status === "A") {
    if (!hasTest) {
      failures.push(`[new file] ${file}: no associated test file found (expected: ${candidateTestFiles(file)[0]} or equivalent).`);
    }
    if (!hasCommentChange) {
    ...
    if (!hasTest) {
      failures.push(`[modified file] ${file}: no associated test file (existing or modified) found.`);
    }
    if (!hasCommentChange) {
      failures.push(`[modified file] ${file}: no comment added/modified/removed in the diff.`);
    }
  }
}
...
if (failures.length > 0) {
  ...
  console.error(
    "\nEvery source file added or modified under backend/src, frontend/src or shared/src must have " +
      "an associated test file (*.test.ts(x) / *.spec.ts(x)) and comments explaining the code.",
  );
  process.exit(1);
}
